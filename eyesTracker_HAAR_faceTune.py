...
startX       = 0.0 #random.uniform(-30.0, 30.0)
n            = math.sqrt(900.0 - startX * startX)
startY       = 0.0 #random.uniform(-n, n)
destX        = startX
destY        = startY
curX         = startX
curY         = startY
moveDuration = random.uniform(0.075, 0.175)
holdDuration = 5.0 #random.uniform(0.1, 1.1)
startTime    = 0.0
isMoving     = False
...
#		print(frames/(now-beginningTime))
			...
			if dt >= holdDuration:
				...
				if not queue.empty():
					if CRAZY_EYES == True :
						...
						scleraMap = pi3d.Texture("graphics/sclera_bloodshot.jpg", mipmap=False, filter=pi3d.constants.GL_LINEAR, blend=True)
						leftEye.set_textures([scleraMap])
						rightEye.set_textures([scleraMap])
						CRAZY_EYES = False
					
					# Update Xdest and Ydest
					queueTime = time.time()
	...
	if CRAZY_EYES:
		...
		newLeftUpperLidWeight = trackingPosR + (n * (1.0 - trackingPosR))
		newLeftLowerLidWeight = (1.0 - trackingPosR) + (n * trackingPosR)
	else:
	...
	# Left eye (on screen right)
 	...
 	# Right eye (on screen left)
	if CRAZY_EYES:
		leftIris.rotateToX(curYR)
		leftIris.rotateToY(curXR - convergence)
		leftIris.draw()
		leftEye.rotateToX(curYR)
		leftEye.rotateToY(curXR - convergence)
	else:
	...
	leftUpperEyelid.draw()	# Decide if I want eye lids
	leftLowerEyelid.draw()
	rightUpperEyelid.draw()
	rightLowerEyelid.draw()
	
	# if(loopCount > 100) :
	# 	irisMap   = pi3d.Texture("graphics/iris_yellow.jpg"  , mipmap=False,
	# 			filter=pi3d.constants.GL_LINEAR)

	# 	rightIris.set_textures([irisMap])
	# 	leftIris.set_textures([irisMap])
  
	# 	scleraMap = pi3d.Texture("graphics/sclera_bloodshot.jpg", mipmap=False,
	# 				filter=pi3d.constants.GL_LINEAR, blend=True)
